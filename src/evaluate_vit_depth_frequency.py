...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate ViT-Small + depth + frequency multi-task model."
    )
    ...
    parser.add_argument("--freeze_backbone", action="store_true",
                        help="Use if the model was trained with --freeze_backbone.")
    return parser.parse_args()


def load_model(checkpoint_path, device, use_attention=True, pretrained=True,
               vit_model_name="vit_small_patch16_224", freeze_backbone=False):
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating ViT+Depth+Frequency"):
            ...
            fake_preds      = torch.argmax(outputs["fake_logits"],      dim=1)
            transform_preds = torch.argmax(outputs["transform_logits"], dim=1)
...
def compute_metrics(results):
    ...
    transform_accuracy = accuracy_score(transform_labels, transform_preds)
    ...
    for tid, tname in enumerate(TRANSFORM_LABEL_NAMES):
        indices = [i for i, l in enumerate(transform_labels) if l == tid]
        if not indices:
            fake_accuracy_by_transform[tname] = None
        else:
            fake_accuracy_by_transform[tname] = accuracy_score(
                [fake_labels[i] for i in indices],
                [fake_preds[i]  for i in indices],
            )
    ...
    return {
        ...
        "fake_classification_report":  classification_report(
            fake_labels, fake_preds, target_names=FAKE_LABEL_NAMES,
            output_dict=True, zero_division=0),
        "transform_classification_report": classification_report(
            transform_labels, transform_preds, target_names=TRANSFORM_LABEL_NAMES,
            output_dict=True, zero_division=0),
    }
...
def save_predictions(results, output_path):
    rows = [{
        ...
        "transform_correct":results["transform_labels"][i] == results["transform_preds"][i],
...
def main():
    ...
    dataset = RRDepthFrequencyDatasetFromCSV(
        ...
        depth_root=args.depth_root, image_size=args.image_size, train=False,
    )
    ...
    model = load_model(
        ...
        use_attention=not args.no_attention, pretrained=not args.no_pretrained,
        vit_model_name=args.vit_model_name, freeze_backbone=args.freeze_backbone,
    )
    ...
    fake_cm = confusion_matrix(results["fake_labels"], results["fake_preds"], labels=[0, 1])
    transform_cm = confusion_matrix(results["transform_labels"], results["transform_preds"],
                                    labels=[0, 1, 2])

    save_confusion_matrix(fake_cm, FAKE_LABEL_NAMES,
                          "Fake Confusion Matrix (ViT+Depth+Frequency)",
                          output_dir / "confusion_fake.png")
    save_confusion_matrix(transform_cm, TRANSFORM_LABEL_NAMES,
                          "Transformation Confusion Matrix (ViT+Depth+Frequency)",
