...
def load_checkpoint(model, checkpoint_path, device):
    ...
    Supports all checkpoint formats used in this project:
        1. {"model_state_dict": ...}   <- saved by FrequencyAugumented.py
        2. {"state_dict": ...}
...
def evaluate_model(model, dataloader, device):
    ...
    The model receives both the RGB image and the FFT log-magnitude map.
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader, desc="Evaluating freq"):
            ...
            for i in range(batch_size):
                rows.append({
                    ...
                    "fake_correct":      int(true_fake[i].cpu())      == int(pred_fake[i].cpu()),
                    "transform_correct": int(true_transform[i].cpu()) == int(pred_transform[i].cpu()),
                })
...
def compute_metrics(predictions_df):
    """
    Compute all evaluation metrics from the predictions DataFrame.

    Returns the same keys as evaluation_depth_frequency.py so that
    show_evaluation_outputs() and the comparison table work unchanged.
    """
    ...
    fake_f1_macro = f1_score(y_true_fake, y_pred_fake, average="macro", zero_division=0)
    ...
    transform_f1_macro = f1_score(
        y_true_transform, y_pred_transform, average="macro", zero_division=0,
    )
    ...
    transform_report = classification_report(
        y_true_transform, y_pred_transform,
...
def save_confusion_matrix(y_true, y_pred, labels, title, output_path):
    ...
    Uses ConfusionMatrixDisplay like evaluate_RGB.py.
    """
...
def print_metrics(metrics):
    ...
    print(f"\nTransform accuracy:   {metrics['transform_accuracy']:.4f}")
...
def parse_args():
    ...
    parser.add_argument("--checkpoint",  type=str, default="checkpoints/best_freq.pt",
                        help="Path to checkpoint saved by FrequencyAugumented.py.")
    parser.add_argument("--output_dir",  type=str, default="results/freq")
    ...
    parser.add_argument("--no_attention",  action="store_true",
                        help="Use if the model was trained without attention gate.")
...
def main():
    ...
    predictions_df = evaluate_model(model=model, dataloader=dataloader, device=device)
